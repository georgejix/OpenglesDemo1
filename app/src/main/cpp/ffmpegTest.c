...
static int write_video_frame(AVFormatContext *oc, OutputStream *ost) {
    ...
    if (ret < 0) {
        LOGI("Error encoding video frame\n");
        exit(1);
    }
    ...
    if (ret < 0) {
        LOGI("Error while writing video frame\n");
        exit(1);
    }
...
Java_com_example_openglesdemo1_ffmpeg_FfmpegUtil_initVideo
    ...
    avformat_alloc_output_context2(&oc, NULL, NULL, output_path);
    if (!oc) {
        LOGI("Could not deduce output format from file extension: using MPEG.\n");
        avformat_alloc_output_context2(&oc, NULL, "mpeg", output_path);
    }
    if (!oc)
        return -1;
    ...
    int num = 0;
    while ((encode_video || encode_audio) && num++ < 500) {
