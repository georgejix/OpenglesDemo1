...
class ChangeFilterActivity : BaseActivity2() {
    private var mInitView = true
    private var mChangeFilterRender: ChangeFilterRender? = null
    ...
    private fun initView() {
        ...
        mChangeFilterRender = ChangeFilterRender(this, object : ChangeFilterRender.Listener {
            override fun onOpenCamera() {
                Log.d(TAG, "MyTest01Render.onOpenCamera")
        ...
        gl_surface.setRenderer(mChangeFilterRender)
    ...
    private fun onClick(view: View) {
        when (view.id) {
            ...
            R.id.tv_change_filter -> gl_surface.queueEvent { mChangeFilterRender?.changeFilter() }
            R.id.tv_take_photo -> mChangeFilterRender?.takePhoto()
        }
    }
    ...
    private fun setCameraSize() {
        ...
        gl_surface.queueEvent {
            mChangeFilterRender?.setMatrix(mUseBackCamera, p)
            openCamera()
        }
    }
