...
class CodecRender(val renderName: String) {
    ...
    private var mOffsetLocation = 0
    private var mVboId = 0
    private var mIboId = 0
    private var mWidth = 0
    private var mHeight = 0
    private var mSavePicTime = 50
    ...
    fun initEgl(sv: Surface, sharedContext: EGLContext) {
        ...
        val vertexId =
            ShaderUtils.compileVertexShader(ResReadUtils.readResource(R.raw.camera2_t13_vertex3))
        val fragmentId =
            ShaderUtils.compileFragmentShader(ResReadUtils.readResource(R.raw.camera2_t13_fragment3))
        ...
        mImgLocation = GLES30.glGetUniformLocation(mProgramId, "s_TextureMap")
        mOffsetLocation = GLES30.glGetUniformLocation(mProgramId, "u_Offset")
    ...
    fun changeView(w: Int, h: Int) {
        ...
        EGL14.eglMakeCurrent(mEglDisplay, mEglSurface, mEglSurface, mEglContext)
        GLES30.glUseProgram(mProgramId)
        GLES30.glViewport(0, 0, w / 4, h * 3 / 2)
        GLES30.glUseProgram(0)
        mWidth = w
        mHeight = h
    }
    ...
    fun draw(textureId: Int, fence: Long) {
        ...
        GLES30.glUniform1f(mOffsetLocation, 1f / (if (0 == mWidth) 1 else mWidth))
        ...
        if (mSavePicTime-- == 0) {
            val buffer = ByteBuffer.allocate(mWidth * mHeight * 3 / 2)
            GLES30.glReadPixels(
                0, 0, mWidth / 4, mHeight * 3 / 2, GLES30.GL_RGBA,
                GLES30.GL_UNSIGNED_BYTE, buffer
            )
            BaseApplication.mContext?.let {
                val fo = FileOutputStream(File(it.getOutputVideoPath("testPic.yuv")).also {
                    if (it.exists()) {
                        it.delete()
                    }
                    it.createNewFile()
