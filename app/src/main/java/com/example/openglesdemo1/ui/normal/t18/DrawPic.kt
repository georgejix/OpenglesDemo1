...
class DrawPic(val sv: SurfaceView, val img: Int) {
    ...
    fun init() {
        ...
        eglSurface = EGL14.eglCreateWindowSurface(
            eglDisplay, eglConfig[0], sv,
            intArrayOf(EGL14.EGL_NONE), 0
        )

        EGL14.eglMakeCurrent(eglDisplay, eglSurface, eglSurface, eglContext)
        val vertexId =
            ShaderUtils.compileVertexShader(ResReadUtils.readResource(R.raw.normal_t18_vertex))
        val fragmentId =
            ShaderUtils.compileFragmentShader(ResReadUtils.readResource(R.raw.normal_t18_fragment))
        ...
        mTextureId = TextureUtils.loadTexture(sv.context, img)
        GLES30.glUseProgram(0)
    }

    fun setMatrix(width: Int, height: Int) {
        Matrix.orthoM(
