...
class TestRenderer(val mContext: Context) : GLSurfaceView.Renderer {
    ...
    init {
        mVetexBuffer = ByteBuffer.allocateDirect(mPoints.size * 4)
            .order(ByteOrder.nativeOrder())
            .asFloatBuffer()
        mVetexBuffer.put(mPoints)
        mVetexBuffer.position(0)
    }
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        GLES30.glShaderSource(
            vertexId, TestUtil.readTextFileFromResource(mContext, R.raw.vertex_test)
        )
        ...
        GLES30.glShaderSource(
            fragmentId, TestUtil.readTextFileFromResource(mContext, R.raw.fragment_test)
        )
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        GLES30.glVertexAttribPointer(aPositionLocation, 2, GLES30.GL_FLOAT, false, 0, mVetexBuffer)
